...
def extract_data(config, **args):
    if config.dt == "benchmark":
        ...
        if config.ds == "cifar10":
            train_dataset = dsets.CIFAR10(root="data/" + config.dt + '/CIFAR10/', train=True, transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010)),]), download=False)
            test_dataset = dsets.CIFAR10(root="data/" + config.dt + '/CIFAR10/', train=False, transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010)),]))
