...
# GENERATE_SEED=42
GENERATE_SEED=40
def load_cifar100(ds, batch_size, device, args):
    test_transform = transforms.Compose(
            [transforms.ToTensor(),
            transforms.Normalize((0.4914, 0.4822, 0.4465), (0.247, 0.243, 0.261))])
    
    temp_train = dsets.CIFAR100(root='../../../data/REDGE/', train=True, download=True, transform=transforms.ToTensor())
    temp_valid = dsets.CIFAR100(root='../../../data/REDGE/', train=True, transform=test_transform)
    ...
    test_dataset = dsets.CIFAR100(root='../../../data/REDGE/', train=False, transform=test_transform)
    ...
    partial_matrix_dataset = CIFAR100_Augmentention_K(traindata, partialY.float(), trainlabels.float(), args.augtype, args.augk)
...
class CIFAR100_Augmentention_K(Dataset):
    def __init__(self, images, given_label_matrix, true_labels, augtype="weak", augk=2):
        self.images = images
        self.given_label_matrix = given_label_matrix
        # user-defined label (partial labels)
        self.true_labels = true_labels
    ...
    def __getitem__(self, index):
        ...
        each_label = self.given_label_matrix[index]
        each_true_label = self.true_labels[index]
        if self.augtype == "weak":
            each_image_a_list = [self.weak_transform(self.images[index]) for _ in range(0, self.augk)]
        if self.augtype == "strong":
            each_image_a_list = [self.strong_transform(self.images[index]) for _ in range(0, self.augk)]
        if self.augtype == "weak+strong":
            each_image_a_list = [self.strong_transform(self.images[index]) for _ in range(0, self.augk // 2)] + \
                                    [self.weak_transform(self.images[index]) for _ in range(0, self.augk - self.augk // 2)]
        if self.augtype == "strong+weak":
            each_image_a_list = [self.strong_transform(self.images[index]) for _ in range(0, self.augk - self.augk // 2)] + \
                                    [self.weak_transform(self.images[index]) for _ in range(0, self.augk // 2)]
        return each_image_o, each_image_a_list, each_label, each_true_label, index
