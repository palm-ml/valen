...
class info_mlp_model(nn.Module):
    def __init__(self, input_dim, output_dim, mid_dim=1000):
        super(info_mlp_model, self).__init__()

        self.L1 = nn.Linear(input_dim, mid_dim, bias=True)
        init.xavier_uniform_(self.L1.weight)

        self.L2 = nn.Linear(mid_dim, mid_dim, bias=True)
        init.xavier_uniform_(self.L2.weight)

        self.L3 = nn.Linear(mid_dim, output_dim, bias=True)
        init.xavier_uniform_(self.L2.weight)
