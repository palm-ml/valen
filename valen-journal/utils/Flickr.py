...
class Flickr(Dataset):
    def __init__(self, txt_path, root_path,  target_transform = None):
        ...
        mat_path = root + '/a_Flickr/Flickr_LDL_config.mat'
        with h5py.File(mat_path, 'r') as data_f_d:
            ...
            # print(type(data_f_d['vote'][:].transpose()))
            # print(self.label_dis)
            # print(type(self.label_dis.shape))
...
def load_Flickr(ds, batch_size, device, has_eval_train=True):
    GENERATE_SEED=52
    print("loading" + ds)
    # target_size = [100,100]
    target_size = [100,100]
    ...
    temp_train = Flickr(root + '/a_Flickr/flickr_train.txt', root + '/a_Flickr/images', target_transform = train_transform)
    data_size = len(temp_train)
    print("total: ", data_size)
    temp_valid = Flickr(root + '/a_Flickr/flickr_train.txt', root + '/a_Flickr/images', target_transform = test_transform)
    ...
    given_label_matrix, avg_C = to_logis(trainlabels, device)
    sio.savemat('tensor.mat', {'logis_label': given_label_matrix})


    # dis_label = trainlabels * logis_label / torch.sum(trainlabels * logis_label, dim = 1, keepdim = True)
    dis_label = trainlabels
    print("average:", avg_C)
    train_dataset = Flickr_Augmentention(traindata, given_label_matrix, dis_label.float())
    ...
    test_loader = None
    print(len(eval_train_loader.dataset), len(valid_loader.dataset))
...
class Flickr_Augmentention(Dataset):
    def __init__(self, images, given_label_matrix, trainlabels):
        self.images = images
        self.given_label_matrix = given_label_matrix
        self.trainlabels = (trainlabels * given_label_matrix) / (trainlabels * given_label_matrix).sum(dim=1, keepdim=True)
        ...
        print("given_label_matrix", self.given_label_matrix) 
